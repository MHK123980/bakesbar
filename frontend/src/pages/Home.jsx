...
export default function Home() {
  const [products, setProducts] = useState(JSON.parse(localStorage.getItem('bakesbar_products')) || []);
  const [config, setConfig] = useState(JSON.parse(localStorage.getItem('bakesbar_config')) || null);
  const [searchQuery, setSearchQuery] = useState('');
  const [cart, setCart] = useState(JSON.parse(localStorage.getItem('bakesbar_cart')) || []);
  ...
  const fetchData = async () => {
    try {
      ...
      if (Array.isArray(prodRes.data)) {
        ...
        localStorage.setItem('bakesbar_products', JSON.stringify(prodRes.data));
      }
      if (confRes.data && !confRes.data.includes?.('<!DOCTYPE html>')) {
        setConfig(confRes.data);
        localStorage.setItem('bakesbar_config', JSON.stringify(confRes.data));
      }
    } catch(err) {
      console.error(err);
    } finally {
  ...
  useEffect(() => {
    localStorage.setItem('bakesbar_cart', JSON.stringify(cart));
  }, [cart]);
  ...
  return (
    <div className="home-container">
          ...
          <>
            ...
            >
              <div className="sidebar-header">
                <h2 className="sidebar-brand">BAKESBAR</h2>
      ...
      <main className="main-content">
        ...
        <div className="products-grid">
          {shouldShowProductLoader && (
            <div className="cookie-loader" role="status" aria-live="polite">
              ...
              <div className="cookie-loader__text">Baking your treats...</div>
            </div>
          )}
        ...
        <p><strong>© 2026 BakesBar All rights reserved By MHK</strong></p>
      </footer>
